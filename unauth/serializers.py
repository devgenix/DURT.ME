...
def generate_url():
    ...
    while AuthModel.objects.filter(short_url=slug):
        slug = ''.join(random.choices(s, k=N)) 
...
class UrlSerializer(serializers.ModelSerializer):
    ...
    def validate_short_url(self, short_url):
        ...
        if AuthModel.objects.filter(short_url=url):
            num = 2
            while AuthModel.objects.filter(short_url=url):
                if num == 100:
    ...
    def create(self, validated_data):
        ...
        url_instance = AuthModel(short_url=short_url, **validated_data)
        url_instance.save()
