...
def Redirect(request, url):
        ...
        try:
            get_url = get_object_or_404(UrlModel, short_url=url)
            long_link = get_url.original_url
            get_url.visits += 1
            get_url.save()
...
def Dashboard(request): 
    ...
    short_urls = UrlModel.objects.order_by('-visits')[0:10]
    return render(request, 'users/dashboard.html', {'Analytics': Data, 'short_urls': enumerate(short_urls, 1)})